...
$.ajax({
		url : "api/home/monthlyReportInflow", context : document.body}).done(function(resp) {
...
var drawMonthly = function() {
	...
	for ( var i in json_response_monthly)
		{
			if(json_response_monthly[i]._id.indexOf("2010") > -1)
				monthlyStorage10.push([json_response_monthly[i].averageInflow ]);
			else if(json_response_monthly[i]._id.indexOf("2011") > -1)
				monthlyStorage11.push([json_response_monthly[i].averageInflow ]);
			else if(json_response_monthly[i]._id.indexOf("2012") > -1)
				monthlyStorage12.push([json_response_monthly[i].averageInflow ]);
			else if(json_response_monthly[i]._id.indexOf("2013") > -1)
				monthlyStorage13.push([json_response_monthly[i].averageInflow ]);
			else if(json_response_monthly[i]._id.indexOf("2014") > -1)
				monthlyStorage14.push([json_response_monthly[i].averageInflow ]);
		}
	...
	$('#container').highcharts({
        ...
        title: {
            text: 'Average Inflow Water in each year (CA)'
        },
        legend: {
            layout: 'vertical',
            align: 'left',
        ...
        series: [{
            name: 'Monthly Inflow - 2010',
            data: monthlyStorage10
        }, {
        	name: 'Monthly Inflow - 2011',
            data: monthlyStorage11
        }, {
        	name: 'Monthly Inflow - 2012',
            data: monthlyStorage12
        }, {
        	name: 'Monthly Inflow - 2013',
            data: monthlyStorage13
        }, {
        	name: 'Monthly Inflow - 2014',
...
$.ajax({
	url : "api/home/yearlyInflowReport", context : document.body}).done(function(resp) {
		json_response_yearly = resp;
		alert('yearly onflow : ' + resp[0]._id);
		drawYearly();
});
...
var drawYearly = function() {
	...
	for (var i in json_response_yearly)
		...
		if(json_response_yearly[i]._id.indexOf("2010") > -1)
			yearlyStorage10 = json_response_yearly[i].averageInflow;
		else if(json_response_yearly[i]._id.indexOf("2011") > -1)
			yearlyStorage11 = json_response_yearly[i].averageInflow;
		else if(json_response_yearly[i]._id.indexOf("2012") > -1)
			yearlyStorage12 = json_response_yearly[i].averageInflow ;
		else if(json_response_yearly[i]._id.indexOf("2013") > -1)
			yearlyStorage13 = json_response_yearly[i].averageInflow ;
		}
